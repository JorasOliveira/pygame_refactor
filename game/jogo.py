...
class Background(
    ...
    def __init__(self):
        ...
        except pygame.error:  # pylint: disable=E1101
...
class Jogador1(pygame.sprite.Sprite):
    ...
    def __init__(self, column, row, block):
        ...
        print(imagem)
        try:  # Importanto a imagem
            player_1_img = pygame.image.load(imagem)  # do jogador 1
        except pygame.error:  # pylint: disable=E1101
...
class Jogador2(pygame.sprite.Sprite):
    ...
    def __init__(self, colum, row, block):
        ...
        print(imagem)
        try:  # Importanto a imagem
            player_2_img = pygame.image.load(imagem)  # do jogador 2
        except pygame.error:  # pylint: disable=E1101
...
def main():  # pylint: disable=R0914 disable=R0912 disable=R0915
    """ Main routine """
    ...
    def calcula_colisao_p_bola(player1, player2, bola):
        if pygame.sprite.collide_mask(player1, bola):
            ...
            if player1.speed_x >= 0:
                ...
                bola.speed_y = (-1) * random.randint(12, 18) + \
                    player1.speed_y * (random.uniform(0.1, 0.7))
                chute.play()
            else:
        ...
        if pygame.sprite.collide_mask(player2, bola):
            ...
            if player2.speed_x <= 0:
                ...
                bola.speed_y = (-1) * random.randint(12, 18) + \
                    player2.speed_y * (random.uniform(0.1, 0.7))
                chute.play()
    ...
    while True:
        ...
        # sai da tela inicial e comeca o jogo
            ...
            for sprite in sprites:
                ...
                # toca o som,
                # aumenta o placar,
                # e reseta todos os sprites para a posicao inicial,
                # com, velocidade X e Y = 0

                calcula_colisao_gol(gol_dir, gol_esq, jabulani)
            ...
            surf.blit(gol_esq.image, [0, 320])  # gol da esquerda
            surf.blit(gol_dir.image, [1177, 320])  # gol da direita

            # Adicionando os placares:
            texto_esquerda = font_score.render(
                f"Ribamar: {placar_esquerda}", True, YELLOW)
            texto_direita = font_score.render(
                f"Messi Careca: {placar_direita}", True, YELLOW)
            ...
            if p_1.score >= 7:
                ...
                texto_vencedor_1 = font_instructions.render(
                    "Jogador 1 venceu!", True, WHITE)
                texto_instrucoes_1 = font_instructions.render(
                    "Pressione 'Enter' para jogar novamente", True, WHITE)
                texto_instrucoes_2 = font_instructions.render(
                    "Pressione 'ESC' para sair", True, WHITE)
                surf.blit(p_1.image, (display_x * 0.47, display_x / 3))
                surf.blit(texto_vencedor_1, [display_x * 4 / 9, display_x / 3])
                surf.blit(
                    texto_instrucoes_1, [
                        display_x * 4 / 11, display_x * 3 / 5])
                surf.blit(texto_instrucoes_2, [
                    (display_x / 2) - (texto_instrucoes_2.get_rect().width / 2),
                    display_x * 11 / 17])
